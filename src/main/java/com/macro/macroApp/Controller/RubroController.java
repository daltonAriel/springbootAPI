...
public class RubroController {
	...
	*/
	...
	public ResponseEntity<?> orderRubro(@PathVariable(value = "id") int proyecto_id, @RequestBody ArrayList<Rubro> listRubros){
		try {
			int i = 1;
	...
	//Actualizar el rendimiento del rubro
	@PatchMapping("/update-rendimiento/{id}")
	public ResponseEntity<?> rendimientoRubro(@PathVariable(value = "id") int proyecto_id, @RequestBody Rubro newRubro){
		try {
			rubroDAO.save(newRubro);	
			return ResponseEntity.status(HttpStatus.OK).build();
		} catch (Exception e) {
	...
	public ResponseEntity<BigDecimal> getSubTotal(@PathVariable(value = "id") int rubro_id){
		try {
			BigDecimal sub = (BigDecimal) rubroDAO.selectSubtotal(rubro_id);
			return new ResponseEntity<BigDecimal>(sub, HttpStatus.OK);
		} catch (Exception e) {
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}
	...
	@GetMapping("/agregado-ecuatoriano/{id}")
	public ResponseEntity<BigDecimal> getAgregadoEc(@PathVariable(value = "id") int rubro_id){
		try {
			BigDecimal sub = (BigDecimal) rubroDAO.selectAgregadoEc(rubro_id);
			if(sub == null) {
				sub = BigDecimal.ZERO;
			}
			return new ResponseEntity<BigDecimal>(sub, HttpStatus.OK);
